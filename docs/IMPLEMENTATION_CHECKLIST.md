# 구현 체크리스트 (회사 운영 기준)

**작성일**: 2025년 1월

---

## 📋 사용자 흐름 기반 구현 순서

### 사용자 시나리오
1. **업로드** → 2. **작업 처리** → 3. **결과 확인** → 4. **다운로드** → 5. **저장소 관리**

---

## ✅ 1단계: 업로드 API + DB 저장 (핵심)

### 백엔드
- [x] `POST /api/videos/upload` - 파일 업로드
- [x] 파일 검증 (확장자 + MIME 타입)
- [x] 파일명 정제 (UUID 사용)
- [x] DB에 Job 생성 (`video_jobs` 테이블)
- [x] `user_id` 저장 (로그인 사용자)
- [x] Rate Limit 적용 (15분당 10회)

### 프론트엔드
- [x] `js/api-upload.js` - 업로드 API 호출
- [x] 업로드 진행률 표시
- [x] 에러 처리

### DB 테이블
- [x] `video_jobs` 테이블
  - `id`, `user_id`, `title`, `original_name`, `video_path`
  - `status`, `progress`, `error_message`
  - `created_at`, `updated_at`, `completed_at`, `expires_at`

### 보안
- [x] 파일 크기 제한 (2GB)
- [x] 파일 타입 검증
- [x] 업로드 경로 직접 접근 차단
- [ ] **사용자별 업로드 쿼터** (추가 필요)

---

## ✅ 2단계: 작업 처리 (Mock → 실제)

### 백엔드
- [x] `simulateJobProcessing()` - Mock 처리
- [x] 상태 전환: `queued` → `processing` → `completed`
- [x] 진행률 업데이트 (`progress`)
- [x] 워커 프로세스 분리 (`worker.js`)
- [x] 동시 처리 제한 (기본 3개)
- [ ] **실제 STT/번역 엔진 연동** (나중에)
- [ ] **재시도 로직** (실패 시)

### 프론트엔드
- [x] `js/job-api.js` - Job 상태 조회
- [x] 폴링 (주기적 상태 확인)
- [x] 진행률 표시
- [ ] **에러 메시지 표시** (추가 필요)
- [ ] **재시도 버튼** (추가 필요)

### DB 테이블
- [x] `video_jobs.status` - 작업 상태
- [x] `video_jobs.progress` - 진행률 (0-100)
- [x] `video_jobs.error_message` - 에러 메시지
- [ ] **재시도 횟수 필드** (추가 필요: `retry_count`)

---

## ✅ 3단계: 저장소 목록/다운로드

### 백엔드
- [x] `GET /api/storage` - 작업 목록 조회
- [x] 사용자별 필터링
- [x] 상태별 필터링 (`all`, `processing`, `completed`, `expiring`)
- [x] 검색 기능
- [x] `GET /api/jobs/:id/subtitle` - 자막 다운로드
- [x] **권한 체크 강화** (소유자만 다운로드)
- [x] 형식별 다운로드 (SRT, VTT, JSON)

### 프론트엔드
- [x] `html/storage.html` - 저장소 페이지
- [x] `js/mypage.js` - 작업 목록 표시
- [x] 필터링/검색 UI
- [x] 다운로드 버튼
- [ ] **만료일 표시** (추가 필요)
- [ ] **에러 메시지 표시** (추가 필요)

### DB 테이블
- [x] `job_results` - 자막 결과 파일 메타
- [x] `video_jobs.expires_at` - 만료일

---

## ✅ 4단계: 7일 만료 삭제

### 백엔드
- [x] `cleanupExpiredJobs()` - 만료 Job 정리
- [x] 스케줄러 (1시간마다 실행)
- [x] 파일 삭제 (업로드 + 결과)
- [x] DB 상태 업데이트 (`deleted`)
- [x] **검증 스크립트** (`scripts/verify-cleanup.sh`)

### 검증
- [x] 만료되었지만 삭제되지 않은 Job 확인
- [x] 삭제 상태이지만 파일이 남아있는 Job 확인
- [x] 로그 기록

---

## ✅ 5단계: 인증/권한 (회사 운영 필수)

### 백엔드
- [x] JWT 토큰 인증
- [x] `authenticateToken` 미들웨어
- [x] 사용자별 데이터 분리
- [x] 다운로드 권한 체크
- [x] 삭제 권한 체크

### 프론트엔드
- [x] `js/token-manager.js` - 토큰 관리
- [x] `js/auth-api.js` - 인증 API
- [x] API 호출 시 토큰 자동 포함

---

## 🚨 추가 필수 항목 (사고 방지)

### 1. 업로드 남용 방지
- [x] Rate Limit (15분당 10회)
- [ ] **IP별 Rate Limit** (추가 필요)
- [ ] **비로그인 사용자 더 강한 제한** (추가 필요)
- [ ] **사용자별 업로드 쿼터** (추가 필요)

### 2. 권한 분리 강화
- [x] 다운로드 권한 체크 (소유자만)
- [x] 삭제 권한 체크 (소유자만)
- [ ] **임시 공유 토큰** (선택적, 나중에)

### 3. 만료 삭제 검증
- [x] 검증 스크립트
- [x] 로그 기록
- [ ] **자동 알림** (삭제 실패 시)

---

## 📊 추가 개선 사항

### 장애 대응
- [x] `error_message` 필드 (DB)
- [ ] **에러 메시지 UI 표시** (추가 필요)
- [ ] **재시도 API** (`POST /api/jobs/:id/retry`) (추가 필요)
- [ ] **재처리 API** (`POST /api/jobs/:id/reprocess`) (추가 필요)

### 용량/비용 관리
- [x] 파일 크기 제한 (2GB)
- [x] 디스크 용량 모니터링
- [ ] **사용자별 업로드 쿼터** (추가 필요)
- [ ] **월별 사용량 추적** (추가 필요)

### 운영 안정화
- [x] 워커 프로세스 분리
- [x] 동시 처리 제한
- [x] 서비스 모니터링
- [x] 자동 재시작

---

## 📝 즉시 추가해야 할 항목

### 1. 에러 메시지 표시 (UI)
**파일**: `html/storage.html`, `js/mypage.js`

```javascript
// 작업 목록에 에러 메시지 표시
if (job.error_message) {
    // 에러 아이콘 + 메시지 표시
    // 재시도 버튼 표시
}
```

### 2. 재시도 API
**파일**: `ax2-api/server.js`

```javascript
// POST /api/jobs/:id/retry
// - status가 'failed'인 Job만 재시도 가능
// - retry_count 증가
// - status를 'queued'로 변경
```

### 3. 사용자별 업로드 쿼터
**파일**: `ax2-api/middleware/upload-quota.js` (신규)

```javascript
// 사용자별 업로드 용량 제한
// - 무료 사용자: 1GB/월
// - 유료 사용자: 10GB/월
// - 비로그인: 100MB/일
```

### 4. IP별 Rate Limit
**파일**: `ax2-api/middleware/rate-limit.js` (수정)

```javascript
// IP별 업로드 제한
// - 같은 IP에서 15분당 5회
// - 로그인 사용자는 더 높은 제한
```

---

## 📚 필수 문서 3개

### 1. 운영 가이드 ✅
- [x] `docs/PRODUCTION_OPERATIONS.md`
- [x] 배포/재시작/로그/장애 확인

### 2. 정책 문서 (추가 필요)
- [ ] `docs/TERMS_OF_SERVICE.md` - 이용약관
- [ ] `docs/PRIVACY_POLICY.md` - 개인정보 처리방침
- [ ] 보관기간 7일, 삭제 정책 명시

### 3. SLA/문의 흐름 (추가 필요)
- [ ] `docs/SUPPORT_GUIDE.md` - 문의 접수 → 확인 → 처리

---

## 🎯 우선순위별 구현 순서

### 즉시 (이번 주)
1. ✅ 업로드 API + DB 저장
2. ✅ 작업 처리 (Mock)
3. ✅ 저장소 목록/다운로드
4. ✅ 7일 만료 삭제
5. ✅ 인증/권한

### 다음 주
6. 에러 메시지 UI 표시
7. 재시도 API
8. 사용자별 업로드 쿼터
9. IP별 Rate Limit

### 나중에
10. 실제 STT/번역 엔진 연동
11. 결제/크레딧 시스템
12. 관리자 페이지
13. S3 스토리지 교체

---

## ✅ 체크리스트 요약

### 완료된 항목 (✅)
- [x] 업로드 API
- [x] 작업 처리 (Mock)
- [x] 저장소 목록/다운로드
- [x] 만료 삭제
- [x] 인증/권한
- [x] Rate Limit
- [x] 파일 검증
- [x] 워커 분리
- [x] 모니터링 스크립트

### 추가 필요 항목 (❌)
- [ ] 에러 메시지 UI 표시
- [ ] 재시도 API
- [ ] 사용자별 업로드 쿼터
- [ ] IP별 Rate Limit
- [ ] 정책 문서
- [ ] 문의 가이드

---

**작성일**: 2025년 1월


